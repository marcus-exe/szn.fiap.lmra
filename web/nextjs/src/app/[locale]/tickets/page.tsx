...
export default function TicketsPage() {
  ...
  const getStatusInfo = (status: number) => {
    const statuses = [t('statuses.open'), t('statuses.inProgress'), t('statuses.resolved'), t('statuses.closed'), t('statuses.cancelled')];
    const colors = [
  ...
  const getPriorityInfo = (priority: number) => {
    const priorities = [t('priorities.low'), t('priorities.medium'), t('priorities.high'), t('priorities.critical')];
    const colors = [
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {tickets.length === 0 ? (
          <div className="bg-black border border-white/10 rounded-lg shadow p-12 text-center">
            <p className="text-gray-400 text-lg">{t('noTickets')}</p>
            <Link
              href="/tickets/create"
          ...
          <div className="grid grid-cols-1 gap-6">
            {tickets.map((ticket) => (
              <div key={ticket.id} className="bg-black border border-white/10 rounded-lg shadow p-6 hover:shadow-lg transition-all">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-gray-300 text-sm mt-1">
                      {t('ticket')}{ticket.id} • {t('created')} {new Date(ticket.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                ...
                <p className="text-gray-300 mb-4 line-clamp-3">{ticket.description || t('noDescription')}</p>
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-400">
                    {t('createdBy')} {ticket.createdByUserId}
                    {ticket.assignedToUserId && <span> • {t('assignedTo')} {ticket.assignedToUserId}</span>}
                  </div>
                  <Link
